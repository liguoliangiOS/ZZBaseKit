...
public extension NSObject {
 // MARK: ------------------------- 判断设备机型 --------------
    ...
    ///适配宽度的系数
     func zz_x(_ size:CGFloat) -> CGFloat {
        return (ZZ_WIDTH_SCALE_FACTOR * size)
    }
    
    ///适配高度的系数
     func zz_y(_ size:CGFloat) -> CGFloat {
    ...
    /// 保存值
     func zz_setUserDefaults(_ key: String, _ value: String) {
        UserDefaults.standard.setValue(value, forKey: key)
    }
    
    /// 取出值
     func zz_getUserDefaults(_ key: String) -> String? {
        return UserDefaults.standard.string(forKey: key) ?? nil
    }
    
    ///删除值
     func zz_removeUserDefaults(_ key: String) {
    ...
    /// 定义16进制值颜色 例如 zz_hexColor("#F64B4B")
     func zz_hexColor(_ hex: String) -> UIColor {
